...
@section('title', __('attributes.areas'))
                    ...
                    <div class="col-12">
                        <div class="page-title-box">
                            <a class="btn btn-success"
                                href="{{ route('admin.areas.create') }}">{{ __('attributes.create') }}</a>
                        </div>
                    </div>
                </div>
                <!-- end page title -->
                                        ...
                                        @foreach ($areas as $area)
                                            <tr id="row-{{ $area->id }}">
                                                ...
                                                <td><input id="checkbox-1" type="checkbox" name="status"
                                                        ...
                                                        data-id="{{ $area->id }}" data-bootstrap-switch
                                                        ...
                                                        <div class="modal-header">
                                                            <h4 class="modal-title" id="myCenterModalLabel">
                                                                @lang('attributes.delete') : <span
                                                                    class="text-danger">{{ $area->name }}</span>
                                                            </h4>
                                                            <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                                aria-label="Close"></button>
                                                        </div>
                                                        <form id="deleteForm{{ $area->id }}"
                                                            action="{{ route('admin.areas.delete', $area->id) }}"
                                                            method="post">
                                                            @csrf
                                                            @method('delete')
