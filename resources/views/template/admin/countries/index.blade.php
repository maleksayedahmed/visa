                    ...
                    <div class="col-12">
                        <div class="page-title-box">
                            <a class="btn btn-success"
                                href="{{ route('admin.countries.create') }}">{{ __('attributes.create') }}</a>
                        </div>
                    </div>
                </div>
                <!-- end page title -->
                                ...
                                <table id="basic-datatable" class="table table-striped dt-responsive nowrap w-100">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>@lang('attributes.country_iso')</th>
                                            <th>@lang('attributes.currency_iso')</th>
                                            <th>@lang('attributes.currency_name')</th>
                                            <th>@lang('attributes.country_code')</th>
                                        ...
                                        @foreach ($countries as $country)
                                            <tr id="row-{{ $country->id }}">
                                                ...
                                                <td><input type="checkbox" name="status"
                                                        ...
                                                        data-id="{{ $country->id }}" data-bootstrap-switch
                                                        ...
                                                        <div class="modal-header">
                                                            <h4 class="modal-title" id="myCenterModalLabel">
                                                                @lang('attributes.delete') : <span
                                                                    class="text-danger">{{ $country->name }}</span>
                                                            </h4>
                                                            <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                                aria-label="Close"></button>
                                                        </div>
                                                        <form id="deleteForm{{ $country->id }}"
    ...
    <script>
        ...
        document.querySelectorAll('input[type="checkbox"][data-bootstrap-switch]').forEach(checkbox => {
            checkbox.addEventListener('change', function() {
                ...
                fetch(`/admin/countries/change-status`, {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                            'Content-Type': 'application/json'
