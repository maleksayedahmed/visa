                                ...
                                <table id="basic-datatable" class="table table-striped dt-responsive nowrap w-100">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>@lang('attributes.postal_code')</th>
                                        ...
                                        @foreach ($cities as $city)
                                            <tr id="row-{{ $city->id }}">
                                                ...
                                                <td><input id="checkbox-1" type="checkbox" name="status"
                                                        ...
                                                        data-id="{{ $city->id }}" data-bootstrap-switch
                                                        ...
                                                        <div class="modal-header">
                                                            <h4 class="modal-title" id="myCenterModalLabel">
                                                                @lang('attributes.delete') : <span
                                                                    class="text-danger">{{ $city->name }}</span>
                                                            </h4>
                                                            <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                                aria-label="Close"></button>
                                                        </div>
                                                        <form id="deleteForm{{ $city->id }}"
                                                            action="{{ route('admin.cities.delete', $city->id) }}"
                                                            method="post">
                                                            @csrf
                                                            @method('delete')
    ...
    <script>
        ...
        document.querySelectorAll('input[type="checkbox"][data-bootstrap-switch]').forEach(checkbox => {
            checkbox.addEventListener('change', function() {
                ...
                fetch(`/admin/cities/change-status`, {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                            'Content-Type': 'application/json'
